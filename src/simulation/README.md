...
Playable runs initialize the backend immediately after generation and commit the profile's configured initial settlement ticks before creating the player or generated containers. Headless Godot reports the GPU backend unavailable and skips this loading-only stage.
