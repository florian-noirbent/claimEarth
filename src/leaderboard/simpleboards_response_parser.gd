...
static func parse_entries(body_text: String):
	var parser := JSON.new()
	...
	for raw_entry in parser.data:
		...
		var entry = _entry_from_dictionary(raw_entry, rank)
		if entry == null:
...
static func parse_entry(body_text: String):
	var parser := JSON.new()
	if parser.parse(body_text) != OK:
...
static func _entry_from_dictionary(source: Dictionary, rank: int):
	...
	if metadata_parser.parse(metadata_text) == OK and metadata_parser.data is Dictionary:
		entry.metadata = metadata_parser.data
	return entry
