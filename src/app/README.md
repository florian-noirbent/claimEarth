...
`RunSession` is disposable per gameplay or preview run. Its controllers keep ownership narrow: `RunWorldController` owns generation, player lifetime, simulation scheduling, camera bounds, and presentation; `RunItemController` owns inventory, projectiles, explosions, and flag resolution; `ScoreController` owns local saves and leaderboard workflows.
