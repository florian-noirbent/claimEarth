...
func test_newer_chunk_job_replaces_and_merges_older_layers() -> void:
	...
	first.configure(Vector2i.ZERO, 1, TerrainLayerMask.FLUID_VISUAL, rect, rect, world.copy_committed_region(rect), metadata, 16.0, 4)
	var second := ChunkBuildJob.new()
	second.configure(Vector2i.ZERO, 2, TerrainLayerMask.SAND_VISUAL, rect, rect, world.copy_committed_region(rect), metadata, 16.0, 4)
