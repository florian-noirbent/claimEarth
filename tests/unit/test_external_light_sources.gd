...
func test_standard_light_sources_validate_overlap_move_and_removal() -> void:
	...
	assert_true(backend.set_high_frequency_light_source(&"flare:first", Vector2i(2, 2), 100, 8))
	assert_true(backend.set_high_frequency_light_source(&"flare:latest", Vector2i(6, 8), 160, 18))
	assert_false(backend.remove_high_frequency_light_source(&"flare:first"))
	assert_true(backend.remove_high_frequency_light_source(&"flare:latest"))
