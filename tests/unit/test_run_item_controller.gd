...
func test_flag_resolution_emits_depth_and_lava_outcome() -> void:
	...
	var position := HexMetrics.center_for_offset(4, 27, 16.0)
	controller.resolve_flag_landing(null, position, null, &"impact")
	assert_signal_emitted_with_parameters(controller, "flag_planted", [27, position])
	controller.resolve_flag_landing(null, position, null, &"lava")
